...
void DisText(uint16 x, uint16 y,uint8 back,uint8 font,u8 *strings )
{
	BEGIN_CMD();
	TX_8(0x20);
	TX_16(x);
...
void SetButtonValue(uint16 screen_id,uint16 control_id,u8 state)
{
	BEGIN_CMD();
	TX_8(0xB1);
	TX_8(0x10);
...
void SetTextValue(uint16 screen_id,uint16 control_id,u8 *str)
{
	BEGIN_CMD();
	TX_8(0xB1);
	TX_8(0x10);
...
void BatchSetText(uint16 control_id,u8 *strings)
{
	TX_16(control_id);
